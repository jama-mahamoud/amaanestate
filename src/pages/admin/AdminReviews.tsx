...
export default function AdminReviews() {
  ...
  // Verify Admin privilege on mount
  useEffect(() => {
    let active = true;
    const unsub = auth.onAuthStateChanged(async (currentUser) => {
      if (currentUser) {
        try {
          ...
          if (active) {
            const isAdm = 
              currentUser.email?.toLowerCase().trim() === 'towinnow0@gmail.com' ||
          ...
          console.error('[Admin Auth Guard Error]:', e);
          if (active) {
            if (currentUser.email?.toLowerCase().trim() === 'towinnow0@gmail.com') {
              setIsAdmin(true);
              loadReviews();
            } else {
              setIsAdmin(false);
  ...
  const stats = useMemo(() => {
    ...
    let published = reviews.filter(r => r.status === 'published').length;
    let rejected = reviews.filter(r => r.status === 'rejected').length;
    let unpublished = reviews.filter(r => r.status === 'unpublished').length;

    return { total, drafts, pending, approved, published, rejected, unpublished };
  }, [reviews]);
      ...
      {!isFormOpen && !previewReview && (
        <div className="space-y-10 max-w-7xl mx-auto animate-fade-in">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div className="bg-[#111116] border border-white/5 p-5 rounded-2xl flex flex-col justify-between border-l-4 border-l-blue-500">
              <span className="text-[10px] font-mono text-neutral-400 uppercase tracking-wider">Approved</span>
              {loading ? (
                <div className="h-6 w-12 bg-white/5 rounded animate-pulse mt-2" />
              ) : (
                <span className="text-2xl font-semibold mt-2 text-blue-500">{stats.approved}</span>
              )}
            </div>
            ...
            <div className="bg-[#111116] border border-white/5 p-5 rounded-2xl flex flex-col justify-between border-l-4 border-l-emerald-500">
              <span className="text-[10px] font-mono text-neutral-400 uppercase tracking-wider">Published</span>
              {loading ? (
                <div className="h-6 w-12 bg-white/5 rounded animate-pulse mt-2" />
              ) : (
                <span className="text-2xl font-semibold mt-2 text-emerald-500">{stats.published}</span>
              )}
            </div>
              ...
              <table className="w-full text-left border-collapse bg-[#111116]">
                ...
                <tbody className="divide-y divide-white/5 text-xs font-light">
                  {filteredReviews.map((rev) => {
                    ...
                    if (rev.status === 'published') badgeClass = 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20';
                    if (rev.status === 'pending') badgeClass = 'bg-amber-500/10 text-amber-400 border border-amber-500/20';
                    if (rev.status === 'approved') badgeClass = 'bg-blue-500/10 text-blue-400 border border-blue-500/20';
                    if (rev.status === 'rejected') badgeClass = 'bg-red-500/10 text-red-500 border border-red-500/20';
                    if (rev.status === 'unpublished') badgeClass = 'bg-neutral-800 text-neutral-500 border border-neutral-700/30';
                            ...
                            <div className="flex items-center bg-black/40 border border-white/5 rounded-lg p-0.5 select-none mr-2">
                              ...
                              {rev.status === 'approved' && (
                                <button
                                  ...
                                  className="px-1.5 py-0.5 text-[8px] bg-[#C5A059] text-black hover:bg-[#D4AF37] font-bold uppercase rounded"
                                  title="Move to live public network page"
                                >
                                  Publish
                                </button>
                              )}
                              ...
                              {rev.status === 'published' && (
                                <button
                                  ...
                                  className="px-2 py-1 text-[9px] text-neutral-400 hover:text-white hover:bg-neutral-800 rounded uppercase font-semibold font-mono"
                                  title="Unpublish from live columns"
                                >
                                  Unpublish
                                </button>
                              )}
                              ...
                              {rev.status === 'unpublished' && (
                                <button
                                  onClick={() => handleUpdateStatus(rev.id, 'published')}
                                  className="px-2 py-1 text-[9px] text-emerald-400 hover:underline hover:bg-neutral-850 rounded uppercase font-semibold font-mono"
                                  title="Publish again to live columns"
                                >
                                  Publish
                                </button>
                              )}

...
export default function ProfessionalModeratedList() {
  ...
  const loadApps = async () => {
    setLoading(true);
    const data = await moderationService.getProfessionalApplications('pending');
    setApps(data);
    setLoading(false);
  };
  ...
  if (apps.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 glass-card rounded-[3rem] border-dashed border-white/5">
        ...
        <p className="text-white/20 text-xs mt-2 uppercase tracking-widest">No pending professional applications found</p>
      </div>
    );
  }
          ...
          <div className="p-8 flex flex-col lg:flex-row items-center gap-10">
            ...
            <div className="flex-1 space-y-2 w-full text-center lg:text-left">
               ...
               <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 text-white/40 text-[10px] font-black uppercase tracking-widest">
                  ...
                  <div className="flex items-center gap-1.5 px-3 py-1 bg-white/5 rounded-lg border border-white/5">
                     <Briefcase size={10} className="text-luxury-gold" /> {app.data.professionalDetails.specialization || 'Generalist'}
                  </div>
               </div>
            ...
            <div className="flex gap-3 shrink-0">
               <Button 
                 ...
                 variant="ghost" 
                 className="h-14 w-14 rounded-2xl bg-white/5 border border-white/5 hover:border-luxury-gold hover:text-luxury-gold"
               >
                ...
                <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-12">
                   ...
                   <div className="space-y-8">
                      ...
                      <div className="pt-6 border-t border-white/5 flex gap-4">
                         <div className="flex-1 p-5 rounded-2xl bg-white/[0.02] border border-white/5">
                            <p className="text-[8px] uppercase font-black tracking-widest text-white/20 mb-1">Email Protocol</p>
                            <p className="text-xs font-bold text-white/80">{app.data.personalInfo.email}</p>
                         </div>
                         <div className="flex-1 p-5 rounded-2xl bg-white/[0.02] border border-white/5">
                            <p className="text-[8px] uppercase font-black tracking-widest text-white/20 mb-1">Comm Channel</p>
                            <p className="text-xs font-bold text-white/80">{app.data.personalInfo.phone}</p>
                         </div>
                      </div>

...
export default function ArticleModeratedList() {
  ...
  const loadArticles = async () => {
    setLoading(true);
    const data = await moderationService.getAllArticles();
    setArticles(data);
    setLoading(false);
  };
      ...
      {articles.map((article) => (
        ...
        >
          ...
          <div className="w-full lg:w-56 aspect-[16/9] rounded-3xl overflow-hidden relative border border-white/10 shrink-0">
            ...
            />
            <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${article.published ? 'bg-green-500 text-white shadow-lg shadow-green-500/20' : 'bg-red-500/20 text-red-500 border border-red-500/20'}`}>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
               <div>
                  ...
                  <div className="flex items-center gap-6 mt-2">
                     ...
                     <div className="flex items-center gap-1.5 text-white/20 text-[9px] font-black uppercase tracking-widest">
                        <Eye size={12} className="text-luxury-gold/50" /> {article.views || 0} Intelligence Pulses
                     </div>
                  </div>
               </div>
